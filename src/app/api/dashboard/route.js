...
export async function GET() {
  try {
    // Run all 9 dashboard queries concurrently in parallel using Promise.all
    const [
      oppsByStage,
      tasksByStatus,
      overdueTasks,
      ...
      // 6. Automation settings threshold
      query('SELECT revision_threshold FROM automation_settings LIMIT 1'),

      // 7. General Stats: Total Opportunities Count
      query('SELECT COUNT(*) FROM opportunities'),

      // 8. General Stats: Active Tasks Count
      query(`
        SELECT COUNT(*) FROM work_items 
        WHERE status_id != (SELECT id FROM dropdown_options WHERE category = 'task_status' AND option_name = 'Completed')
      `),

      // 9. General Stats: Total Pipeline Value
      query(`
    ...
    // 10. Rework Hotspots query using resolved threshold
